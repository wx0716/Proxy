...
Service_mtproxy(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/wx0716/Proxy/master/mtproxy_centos.sh" -O /etc/init.d/mtproxy; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/wx0716/Proxy/master/mtproxy_debian.sh" -O /etc/init.d/mtproxy; then
			echo -e "${Error} MTProxy服务 管理脚本下载失败 !"
			rm -rf "${file}"
			exit 1
		fi
